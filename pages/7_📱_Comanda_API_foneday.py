...
def step4_add_to_cart():
    ...
    try:
        ...
        # MODIFICAT: folosește sale_price când există, altfel regular_price
        cursor.execute("""
            SELECT fi.product_id,
                   ...
                   COALESCE(NULLIF(wp.sale_price, 0), wp.regular_price) AS woo_price_ron,
                   wp.sale_price AS woo_sale_price
        ...
        df_foneday = pd.DataFrame(
            ...
            columns=[
                ...
                "foneday_sku",
                "foneday_price_eur",
                "woo_price_ron",      # preț efectiv: sale sau regular
                "woo_sale_price",     # preț promoțional brut
            ],
        )
        ...
        df_foneday["woo_sale_price"] = pd.to_numeric(
            df_foneday["woo_sale_price"], errors="coerce"
        )

        # Marcăm produsele care sunt la reducere (sale_price nu e NULL / 0)
        df_foneday["is_on_sale"] = df_foneday["woo_sale_price"].notna() & (df_foneday["woo_sale_price"] > 0)
        ...
        edited = st.data_editor(
            ...
            column_config={
                ...
                "is_on_sale": st.column_config.CheckboxColumn(
                    "🔥 Reducere Woo?",
                    help="Bifat = produsul are sale_price în WooCommerce",
                    disabled=True,
                ),
            },
            hide_index=True,
        ...
        # Avertizare pentru produsele la reducere
        num_on_sale = int(df["is_on_sale"].sum())
        if num_on_sale > 0:
            st.warning(
                f"⚠️ **ATENȚIE:** {num_on_sale} produse din tabel au preț promoțional în WooCommerce (sale_price). "
                "Verifică dacă vrei să le comanzi din nou sau doar să faci dump de stoc."
            )
...
# NAVIGARE ÎN PAGINĂ (Task 2 - scos din sidebar)
st.title("📱 Comanda API Foneday")
...
2. Pentru fiecare produs:
   - Obține prețul de vânzare din `v_woo_prices` (WooCommerce) – **folosește sale_price dacă există, altfel regular_price**
...
3. Pentru produsele profitabile:
   ...
   - **Avertizează** vizibil produsele care au sale_price activ (reducere în WooCommerce)
...
- Avertizare clară pentru produse la reducere (sale_price)
...
```
    ...
    try:
        ...
        if conn:
            ...
            if not df.empty:
                st.info(f"🗺️ Găsite {len(df)} mapări (primele 200)")
                df['last_verified_at'] = pd.to_datetime(df['last_verified_at']).dt.strftime('%Y-%m-%d %H:%M:%S')
    ...
    st.markdown("### 📝 Log Complet Evenimente")
    ...
    try:
        ...
        if conn:
            ...
            if not df.empty:
                st.info(f"📝 Afișate ultimele {len(df)} evenimente")
                df['created_at'] = pd.to_datetime(df['created_at']).dt.strftime('%Y-%m-%d %H:%M:%S')
